...
export default function FeatureCard({ title, description, linkText, linkUrl, color = "green" }) {
    return (
        <div className={`group bg-white p-8 rounded-2xl border border-gray-100 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1`}>
            ...
            <Link href={linkUrl || "#"} className="text-sm font-bold uppercase tracking-wider text-green-600 hover:text-green-700 border-b-2 border-transparent hover:border-green-600 transition-all inline-block pb-1">
                {linkText}
            </Link>
        </div>
    );
